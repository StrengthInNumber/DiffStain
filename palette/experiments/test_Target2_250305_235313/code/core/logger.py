...
class VisualWriter():
    ...
    def set_iter(self, epoch, iter, dir_name=None, phase='train'):
        self.phase = phase
        self.epoch = epoch
        self.iter = iter
        self.dir_name = dir_name

    def tensor_to_img(self, ts):
        img_arr = ts.numpy()
        img_arr = (img_arr - img_arr.min()) / (img_arr.max() - img_arr.min()) * 255
        return img_arr.astype('uint8')
        
    def save_images_cell_painting(self, results, result_path):
        names = results['name']
        for i in [0, 2]:
            imgs_tensor = results['result'][i]
            for j in range(len(imgs_tensor)):
                # print(i, j, imgs_tensor.shape)
                Image.fromarray(self.tensor_to_img(imgs_tensor[j])).save(os.path.join(result_path, names[i] + 'C0{}.png'.format(j+1)))
