...
class Palette(BaseModel):
    def __init__(self, networks, losses, sample_num, task, optimizers, t_start=None,
        ...
        if network_ckpt is not None:
            print('*************** \n Loading network from checkpoint: ', network_ckpt)
            state_dict = torch.load(network_ckpt)
    ...
    def set_input(self, data):
        ...
        self.t_start = int(data['t_start'][0])
        self.path = data['name']
        self.batch_size = len(data['name'])
        self.dir_name = data['name'][0]
    ...
    def test(self):
        ...
        with torch.no_grad():
            for phase_data in tqdm.tqdm(self.phase_loader):
                ...
                self.iter += self.batch_size
                self.writer.set_iter(self.epoch, self.iter, self.dir_name, phase='test')
                for met in self.metrics:
                    key = met.__name__
                    value = met(self.gt_image, self.output)
